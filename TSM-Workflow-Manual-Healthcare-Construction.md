...
## Additive layer: HC Office Manager → Neural Intake

**Where:** `html/healthcare/hc-office-manager-doc-intake.html` (browser
bridge), `server/healthcare/hc-neural-intake.js`,
`server/healthcare/hc-node-registry.js`,
`server/healthcare/hc-node-contract.js`. Introduced in commit `7c172c77`.

This is a second, additive routing path that runs alongside Stage 4 for
healthcare documents specifically. It does not replace `routeDocument()` —
both the module's own header comment and the browser bridge's inline
comments state this explicitly, and the wiring below backs it up.

**Advisory routing (browser-side):** `window.hcOmRouteDocument(text)`
lowercases the document text and scores it against a per-node keyword-term
table (`cfg.routing`), returning `[{ node, score }, ...]` sorted highest
score first — e.g. a document mentioning compliance and vendor terms scores
both `compliance` and `vendors`. `window.hcOmBuildRoutingEnvelope()` wraps
those routes with `strategist`/`portal` destinations and a
`HC_CROSS_NODE_BNCA` mode tag.

**Browser bridge:** `window.TSM_HC_OM_NEURAL_INTAKE.buildEnvelope()`
normalizes those advisory routes into a `candidateNodes` array — keeping
only routes with a `node` string and `score > 0`, deduplicated. If no
routes were passed in, it calls `hcOmRouteDocument()` itself so
`candidateNodes` isn't empty just because the caller hasn't populated
`advisoryRoutes` yet.

**Wiring into the uploader:** inside `processFile()`, two `try/catch`
blocks (each explicitly non-fatal — a caught error just logs a warning and
leaves existing routing untouched) do the following:
1. Build the Neural Intake envelope and attach it at
   `classification.routing.healthcare.hcNeuralIntake`.
2. Call `hcOmBuildRoutingEnvelope()`, then merge its suggested nodes into
   `classification.routing.healthcare.nodes` — **existing classifier
   routes first, HC OM suggested nodes appended**, deduplicated. The
   suggested routes (with scores) are preserved separately at
   `routing.hcOmSuggested` for UI/debugging, and folded into the Neural
   Intake envelope as `suggestedRoutes`.

**Server-side intake (`hc-neural-intake.js`):**
`collectCandidateNodes()` merges candidate nodes from up to four possible
sources on the input — `suggestedNodes`, `routing.nodes`,
`routing.advisoryRoutes` (extracting `.node` from each scored route), and
`routing.healthcare.nodes` — deduplicates them, and keeps only IDs that
exist in the node registry (`registry.hasNode()`). `createIntakeEnvelope()`
then builds one standardized **node request**
(`contract.createNodeRequest()`) per candidate node, each carrying the
document, extraction, classification, and routing data plus a `context`
block (source + persona).

**Node registry:** 11 registered nodes, all currently `active: true` —
`billing`, `compliance`, `financial`, `grants`, `insurance`, `legal`,
`medical`, `operations`, `pharmacy`, `taxprep`, `vendors`.
`registry.validateRegistry()` checks the count is exactly 11 and every
entry's `id` matches its own key.

**Findings and Strategist handoff:** `collectNodeFinding()` normalizes a
node's result into a fixed shape (`relevance`, `confidence`, `findings`,
`risks`, `recommendations`, `evidence`) via
`contract.createNodeResult()`, and throws if the node isn't registered.
`correlateFindings()` keeps only findings with nonzero relevance, or at
least one finding/risk, and summarizes each as counts.
`buildStrategistEnvelope()` packages the candidate nodes, findings, and
correlations into a Strategist-bound envelope: destination
`/html/healthcare/hc-strategist/index.html`, mode `HC_CROSS_NODE_BNCA`.

**Why it matters for revenue recovery:** the existing Stage 3/4 classifier
routing was built around a single `exclusionCode`/`documentType` decision
tree (see the collision risk documented below) — it wasn't designed to say
"this document touches both Compliance and Vendors." The advisory layer's
keyword scoring lets a document fan out to every HC domain it plausibly
touches, without either replacing or gating on the classifier's decision.
Because both integration points are wrapped in non-fatal try/catch, a bug
or missing dependency in this layer degrades to "no advisory routing this
time," not a broken upload.

**Verification:** `scripts/test-hc-neural-intake.sh` exercises this path
end-to-end — candidate-node collection, node-request generation, the
`compliance`/`vendors` scored-routing case, browser-side routing, and the
Strategist destination — against the connected system.

**Known gap:** the browser bridge's `portal` destination points to
`/html/healthcare/hc-om-portal.html`, which does not exist in this repo
yet. The Neural Intake envelope and Strategist handoff do not depend on
that file, so this doesn't block the routing path above — but the
"⑥ REVIEW IN OM PORTAL" step in the on-page HC OM guide currently has no
page to land on.
